...
o.p="//static.hsappstatic.net/head-dlb/static-1.979/"
...
Object.defineProperty(t,"__esModule",{value:!0})
t.default=void 0
const n=["api","local","app","private","platform","tools","meetings"],o=["hubspotstarter","hubspotfree"],a=["growth"],i={com:["hubspot","hubteam","grader","getsignals","getsidekick","gettally","hubspotemail","customer-hub","hubspotservicehub","hubspotquote"].join("|"),net:o.join("|"),org:a.join("|")},s=function e(t){const r=new RegExp(`^(?!local|test|selenium)(.*\\.)?(${Object.entries(i).map(([e,t])=>`(?:${t})(qa)?\\.${e}`).join("|")}|(?:connect)\\.com|(?:connect)(qa)\\.co|wthubspot\\.(com|de|es|fr|jp))$`).test(t.hostname),o=new RegExp(Object.entries(i).map(([e,t])=>`(?:${t})qa\\.${e}`).join("|")+"|(?:connect)qa\\.co|wthubspot\\.(com|de|es|fr|jp)"),a=new RegExp(`^(?:${n.join("|")})-(.*).(?:hubspot|hubteam)(?:qa)?.com`),s=new RegExp("^(.*).(?:hubspotquote)(?:qa)?.com"),l="ENV",u="Enviro error: the default argument for .get and .getShort is no longer supported",c=e=>{let t=window[e]
...
return e||(a.test(t.hostname)?a.exec(t.hostname)[1]:s.test(t.hostname)?s.exec(t.hostname)[1]:"na1")},getInternal:v,getShort:m,isProd:_,isQa:y,normalize:h,set:function(e,t){if(1===arguments.length){t=e
...
try{t?localStorage.setItem("DEBUG",JSON.stringify(t)):localStorage.removeItem("DEBUG")}catch(e){d("DEBUG",t||void 0)}}}}}
...
//# sourceMappingURL=//static.hsappstatic.net/head-dlb/static-1.979/bundle.production.js.map